...
- Keep your style scoped
  ...
  * Global style should only go in `src/assets/styles`, never in components
  * Avoid style interactions between components, if some style may need to be shared, refactor it as a framework
    component in put it in the global style.
...
- When ugly hacks cannot be avoided, create an explicit `src/assets/styles/hacks.scss` file and put it in:
